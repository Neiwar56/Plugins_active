		...
		if ( $product->is_type( 'variable' ) || $product->is_type( 'grouped' ) ) {
			...
			foreach ( $product->get_children() as $child_id ) {
				...
				if ( $variation_sale_price ) {
					...
					$new_variation_percent_sale = ( 100 - round( ( $variation_sale_price * 100 ) / $variation_regular_price ) );
					$variation_percent_sale     = max( $new_variation_percent_sale, $variation_percent_sale );
				}
			}
			if ( $variation_percent_sale > 0 ) {
				return '<span class="qodef-woo-product-mark qodef-woo-onsale">' . '-' . $variation_percent_sale . '%' . '</span>';
			} else {
				return '';
			}
